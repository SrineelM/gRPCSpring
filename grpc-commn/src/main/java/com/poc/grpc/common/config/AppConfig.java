 ...
 * This class defines beans for various application-wide concerns such as
 * JSON serialization, asynchronous task execution, time handling, and more.
 *
...
@RequiredArgsConstructor // Injects final fields via constructor (e.g., Environment)
public class AppConfig {
    ...
    /**
     * Provides a primary, application-wide Jackson ObjectMapper for consistent JSON processing.
     * This configuration ensures that all JSON serialization and deserialization follows
     * defined standards (snake_case, proper Java 8 time handling, etc.).
     *
    ...
    public ObjectMapper objectMapper() {
        ...
        // Serialize dates as ISO-8601 strings (e.g., "2023-10-27T10:00:00Z") instead of numeric timestamps.
    ...
    /**
     * Provides a Clock bean that is timezone-aware.
     * Using a Clock bean instead of `LocalDateTime.now()` directly allows for easier testing,
     * as the clock can be mocked to provide a fixed time. It also ensures time consistency.
     *
    ...
    /**
     * Configures the default thread pool for executing methods annotated with @Async.
     * This executor is used for general-purpose asynchronous tasks.
     *
    ...
    /**
     * Configures a dedicated thread pool for Saga orchestration tasks.
     * Using a separate executor for Sagas prevents long-running saga steps from
     * starving the general-purpose async task pool, improving resource management and resilience.
     *
     * @return A configured Executor for saga-related async tasks.
     */
    @Bean(name = "sagaTaskExecutor")
    public Executor sagaTaskExecutor() {
        log.info("Configuring saga orchestration task executor - Core: {}, Max: {}, Queue: {}",
                sagaAsyncCorePoolSize, sagaAsyncMaxPoolSize, sagaAsyncQueueCapacity);
    ...
    /**
     * Provides a primary JSR-303 Validator bean for use across the application.
     * This ensures consistent validation logic (e.g., in @Valid annotations in controllers).
     *
    ...
    /**
     * Provides a RestTemplate bean for making synchronous HTTP client calls.
     * It is pre-configured with connection and read timeouts to prevent
     * application threads from hanging on slow network responses.
     *
    ...
    /**
     * Provides a utility bean for generating unique correlation IDs.
     * These IDs are useful for tracing requests as they flow through different
    ...
    /**
     * Checks if the "dev" Spring profile is active.
     * This is a utility method used to enable development-only features,
     * such as pretty-printing JSON.
     *
    ...
    /**
     * A simple utility class for generating UUID-based correlation IDs.
     * Defined as a static nested class because it doesn't need access to AppConfig's instance state.
     */
    ...
    /**
     * A custom, domain-specific event publisher that acts as a facade over Spring's
     * underlying ApplicationEventPublisher. This promotes a cleaner, domain-oriented
     * approach to publishing events.
     */
    @Component
    ...
    /**
     * A custom application event class representing events related to the Order domain.
     * It extends Spring's ApplicationEvent and carries a specific type and data payload.
     */
